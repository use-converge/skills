...
[MCP guide](references/mcp.md). Use `https://app.use-converge.com/mcp` with an
`Authorization: Bearer cvg_...` key that has `api.full`; it is HTTP-only, never
session-cookie or stdio based, and does not use OAuth discovery. Require an
explicit `idempotency_key` for
...
- Treat upload admission, extracted-text retention, native-file delivery, text-fallback excerpting, and model-visible completeness as separate limits. Converge accepts up to the documented upload cap and stores extracted text for provenance/fallback. It may deliver native files to first-party providers when supported, but text-fallback providers, unsupported provider/media combinations, and later review/synthesis/chairman stages may receive only selected excerpts. Different models in the same run can therefore see different portions of a long upload. Native-file providers may also enforce their own file-token limits. In a five-member production council that included Gemini 3.5 Flash, synthetic ASCII Markdown succeeded through gathering, review, chairman synthesis, and summary at 5,505,024 bytes and failed at 5,570,560 bytes because Gemini rejected the native-file input as over its 1,048,576-token limit. The current Converge provider-size matrix therefore blocks Gemini stacks above 5 MB. For unattended five-member production council runs with Gemini in the stack, keep Markdown uploads at or below 5 MB unless the task explicitly calls for a capped boundary test. The practical ceiling is content-dependent because provider tokenization varies by document format and text.
- Use document uploads for source fidelity, summaries, extraction, and broad review. For exhaustive artifact review, claim-by-claim audits, adversarial fact-checks, or completeness-sensitive reviews, do not rely on an upload alone unless you can verify the relevant reviewers saw the needed material.
- For exhaustive review of an artifact larger than the question cap, build an inline claims digest instead of uploading the full artifact as the only source: extract every checkable claim, quote or tightly paraphrase it, number claims, group them by theme, and drop navigation, styling, repeated prose, and non-reviewable filler. Keep the digest under the 20,000-character question limit and end it with a distinctive tail sentinel such as `END OF DIGEST.` that reviewers must confirm. This is not the banned paste-the-whole-document workaround; it is a deliberate complete enumeration of reviewable claims at a fraction of the artifact size.
- When the digest run is a rerun after a partial-visibility round, briefly list the prior round's useful findings in the packet. Tell reviewers to prioritize new issues over re-litigating known ones, and to confirm or refute prior findings only where they disagree.
...
- For document-size or document-visibility triage, inspect `.documents[]` for `byte_size`, `extracted_char_count`, `text_truncated`, and `blob_available`, then inspect per-output status and `document_input_mode` / `document_input_note` fields when the JSON exposes them. A run that reaches chairman output can still be `partial_failed` if one council member failed in gathering; do not call that a clean no-error size.
...
- Distinguish question truncation, share rendering limits, text-fallback excerpting, and native-file provider limits. If the JSON/share question tail is intact but reviewers describe seeing only excerpts of an uploaded document, or some models quote document tail material while others say they received only excerpts, the submitted question likely arrived intact and the upload was only partially visible to some stages or providers. For exhaustive review, rebuild the packet as an inline claims digest and rerun once, carrying prior findings forward for deduplication instead of resending the same full upload.
